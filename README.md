...
Customize sensitivity thresholds and export SARIF/JSON logs:
```bash
aegis scan /path/to/model.safetensors \
...
### Dynamic Trojan Fuzzing (CLI)
Run perturbation fuzzing on PyTorch models to detect Sleeper Agent Trojan activation spikes:
```bash
aegis fuzz /path/to/model.pt \
  --iterations 50 \
  --spike-threshold 4.0 \
  --output-sarif fuzz_report.sarif
```
...
- name: Run Aegis Security Scan
  run: aegis scan models/weights.safetensors --output-sarif results.sarif
...
- [Engineering Design: Issue #7 (CLI & Terminal UX)](docs/ISSUE_7_ENGINEERING_DESIGN.md)
- [Engineering Design: Issue #8 (SARIF 2.1.0 & CI/CD Pipeline)](docs/ISSUE_8_ENGINEERING_DESIGN.md)
